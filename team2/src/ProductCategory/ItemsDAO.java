...
public class ItemsDAO {
	...
	public List<ItemsVO> getAllContents(int pageNO, int listSize, String ord, String setCategory) {
		...
		try {
			...
			if(ord.equals("신상품순") || ord.equals("") || ord == null){
				String query = "SELECT * FROM "+setCategory+" order by regdate desc "+" LIMIT "+pageNO+","+listSize;
				pstmt = conn.prepareStatement(query);
			}else if(ord.equals("낮은가격순")){
				String query = "SELECT * FROM "+setCategory+" order by productprice "+" LIMIT "+pageNO+","+listSize;
				pstmt = conn.prepareStatement(query);
			}else if(ord.equals("높은가격순")){
				String query = "SELECT * FROM "+setCategory+" order by productprice desc "+" LIMIT "+pageNO+","+listSize;
				pstmt = conn.prepareStatement(query);
			}
...
//  -------- 전체 상품 개수 조회 ------- //
	public int getTotalCount(String setCategory) {
		int num = 0;
		try {
			conn = DBConnection.getConnection();
			String query = "SELECT count(*) FROM "+setCategory;
			pstmt = conn.prepareStatement(query);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				num=rs.getInt(1);
...
//  -------- 상품 등록 ------- //	
	public int insertContent(ItemsVO vo) {
		int num = 0;
		String sql ="";
		String setCategory = vo.getCategory();
		try {
			conn = DBConnection.getConnection();
			sql = "select max(num) from "+setCategory;
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()){ num = rs.getInt(1) + 1;
			}else{ num = 1; }

			String query = "INSERT INTO "+setCategory
			...
			pstmt.setString(12, setCategory);
...
//  -------- 특정 상품 조회 ------- //
	public ItemsVO getContent(int num, String setCategory) {
		ItemsVO vo = new ItemsVO();
		
		try {
			conn = DBConnection.getConnection();
			String query = "SELECT * FROM "+setCategory+ " WHERE num = ?";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if(rs.next()) {
...
//  -------- 상품 수정 ------- //		
	public int updateContent(ItemsVO vo) {
		...
		try {
			conn = DBConnection.getConnection();
			String query = "UPDATE "+setCategory+" SET productName=?,productContent=?,sellerName=?,productPrice=?,"
...
//  -------- 상품 삭제 ------- //	
	public int deleteContent(int NUM, String setCategory) {
		...
		try {
			conn = DBConnection.getConnection();
			String query = "DELETE FROM "+setCategory+" where NUM = ?";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, NUM);
			pstmt.executeUpdate();					
	...
	public int insertNewReply(ItemsRepVO repVO, String setCategory) {
		int rnum = 0;
		String sql ="";
		...
		try {
			...
			String query = "INSERT INTO "+setCategory+"_rep(num, pw, content, writer)"
					+ "VALUES(?, ?, ?, ?)";
