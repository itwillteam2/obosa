...
$(document).on(   "click", ".hover>.cart", function() {
   if("${id}"==""){alert("회원 전용 서비스 입니다.");   }
		...
		var _pnum =$(this).attr("data-pnum");
		
		$.post("${contextPath}/cart/addCart.do",
				{category:_category,pnum:_pnum,cartQuantity:1})
				.done(function(result){if(result=="ok"){ $('.addCart').html('<span class="event"></span>');
				      cartCount();
			     }else{alert("다시 시도해 주시기 바랍니다.")}
				})
				.fail(function(){alert("서버와의 연결에 실패하였습니다")});
   }
});
