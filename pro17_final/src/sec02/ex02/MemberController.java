...
//삭제하는 과정
//1. 회원목록창(listMembers.jsp)에서 삭제 를 클릭해 요청명 /member/delMember.do주소와
//   삭제할 회원 ID를 컨트롤러로 전달 
//2. 컨트롤러는 request객체의 getPathInfo()메소드를 이용해 요청명 /delMember.do을 가져옵니다.
//3. 삭제할 회원 ID를  SQL문으로 전달해 DELETE문 완성후 t_member테이블에 회원정보를 삭제 합니다.
//4. DELETE에 성공하면  성공한 msg값을 request영역에 저장후 
//   다시 컨트롤러로 /member/listMembers.do요청명으로 재요청해 모든회원정보 조회후 
//   listMembers.jsp로 포워딩 하면서 조회된 회원목록을 출력합니다.
...
public class MemberController extends HttpServlet{
	...
	protected void doHandle(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//삭제요청 주소를 받았을때...
			...
			//삭제후 회원목록창(listMembers.jsp)으로 삭제잡업완료 메세지를 전달하기위해
			//request내장객체영역에 바인딩(저장)함.
			req.setAttribute("msg", "deleted");
			
			//삭제후 모든회원을 조회 하는 요청을 서블릿으로 하기 위해..
			//nextPage변수에 요청주소 저장
			nextPage = "/member/listMembers.do";
		}else{
			List membersList = memberDAO.listMembers();
